...
async function callGeminiAPI(prompt, shouldGenerateFeedback) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            let errorMessage = responseBody?.error?.message || `Gemini API request failed with status ${response.status}`;
        ...
        if (responseBody?.candidates?.[0]?.finishReason === 'SAFETY') {
            ...
            throw new Error(`The generated response was blocked due to safety concerns${blockedCategory ? ` (${blockedCategory})` : ''}. Please try again or rephrase.`);
        }
...
app.post('/api/interview-chat', async (req, res) => {
    ...
    try {
        ...
        res.json({ nextBotMessage: generatedText, isComplete: shouldGenerateFeedback });
    } catch (error) {
